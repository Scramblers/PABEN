...
## Agency as Situated Continuation
...
The model is organized around five functional domains: Perception, Action, Beliefs, Experience, and Needs. These domains form the acronym PABEN.

<p align="center">
    <img src="../figures/01-Main-Flow.png" alt="PABEN overview" width="80%">
</p>

*Figure 1: The PABEN domains and their main inter-domain flow. Need-pressure opens Experience, Experience releases a Belief-field, Belief-expectancy selects an intended Belief, and Action tests the corresponding Skill in the perceived Context.*
...
## Belief-Fields and Belief-Horizons
...
For example, a hungry animal with a short Belief-horizon may act only toward visible food. An agent with a longer Belief-horizon can maintain a path such as finding tools, asking for help, preparing food, or waiting for a future meal, because the higher-level belief remains stable while immediate contexts and skills change.

## Learning from Valence Backwards

The model does not learn meaning from neutral structure alone. It learns from valence backwards. Pain, relief, appetite, satisfaction, threat, and fulfillment mark the beliefs and contexts where the world becomes significant. From these valenced outcomes, Experience can bind what happened before: which context was present, which skill was used, which belief was applied, and what continuation became possible.

In this sense, the agent does not first build a complete neutral model of the world and then assign value to it. It discovers valence-bearing situations and builds structure backward from them.

A new belief therefore becomes meaningful only if it can hook into an already known valence-bearing Belief-field. Reflection searches for such hooks: it tries to connect the present unstable or unresolved situation to some known path where fulfillment, relief, avoidance, repair, or continuation has previously been possible. If no hook can be found, the belief may remain empty, speculative, or unusable. If a hook is found, the agent can extend its Belief-horizon: it can now maintain a longer or more abstract path from the present context toward what matters.
...
|---|---|
| Trustability / T | Does the required context hold? |
| Executability / Q | Can the required skill be carried out? |
| Reachability / R | Does the path offer continuation from the present situation? |
| Valencability / V | Does the path preserve access to what matters? |

Together, these dimensions determine whether a belief is strong enough to guide action.

The Belief-field is continuously evaluated through expectancy. Each available belief has its own path viability, and the field as a whole defines the agent's current action horizon. When one belief has sufficiently high expectancy and fits the active need-state, the agent can act fluently. When several conflicting beliefs compete, attention may become unstable or divided. When the field loses expectancy, the agent cannot simply continue. It must explore, reflect, wait, ask for help, and form new beliefs.

## Process Modes

The agent does not normally calculate all this consciously. Most of the time, it simply continues. A context is recognized, a Belief-field is opened, one belief is dominant, a skill is released, and the agent moves through a familiar action-pattern. This ordinary mode may be called Recognize-Execute.

The three main process modes can be understood intuitively as three directions of continuation: forward, sideways, and upward.

Recognize-Execute moves forward. It uses already stabilized context-skill structures to continue along a path that is sufficiently known. In learned situations, the agent does not need to reflect on every step. Perception and Action are already coordinated through stable beliefs and experiences, so the agent can move directly from recognized context to released skill.

Try-Observe moves sideways. It holds the current context in attention and varies action in order to learn what the context can bear, what changes with action, and which variants belong together. This is the process behind exploration, play, practice, and curiosity. The agent does not yet leave the local situation; it moves around within it, testing nearby variants and expanding the usable context-skill field.

Reflection moves upward. It searches for hooks, builds new beliefs, reorganizes paths, or constructs a wider route when the current Belief-field does not contain a sufficiently viable continuation. Reflection works on the Belief-horizon: it tries to find a higher-level belief that can hold the situation together while lower-level contexts, skills, and sub-beliefs are reorganized beneath it.

The three processes therefore have different roles. Recognize-Execute carries the agent forward through an already viable path. Try-Observe expands the local field sideways by acting and observing. Reflection moves upward to search for a wider organizing belief when no available belief is strong enough. Reflection does not create meaning from nothing; it attempts to connect the present situation to a known or possible valence-bearing field. It is therefore not only problem-solving, but hook-finding into known valence-space.


## Emotion, Attention, and Consciousness

Attention is guided by expectancy changes. The agent does not attend to everything equally. Attention is drawn toward the belief, context, skill, or need-state where the unresolved change in expectancy matters most. Within the Belief-field, attention selects the belief whose consequences are most urgent, promising, threatening, unstable, or unresolved. If the path is stable, attention can remain light or embedded in action. If the path becomes unstable, attention tightens around the problem.

Consciousness is therefore treated as a limited mode of occupation: the place where the agent must stabilize, redirect, or reconstruct its continuation.
